...
if [ 1 ==  '1' ]
then
   cppad_lib_path='/home/team07/libraries/CppAD/build/cppad_lib/libcppad_lib.so'
else
   cppad_lib_path='/home/team07/libraries/CppAD/build/cppad_lib/libcppad_lib.a'
fi
...
#
# compile command
compile_command="/usr/bin/c++ test_one.cpp -o test_one.exe
   $dir/$file $extra
   -g
   $cxx_flags
   $include_flags
...
# --------------------------------------------------------------------------
# LD_LIBRARY_PATH
my_path='/home/team07/libraries/CppAD/build/cppad_lib'
if ! echo $LD_LIBRARY_PATH | grep "$my_path" > /dev/null
then
   if [ "$LD_LIBRARY_PATH" == '' ]
   then
