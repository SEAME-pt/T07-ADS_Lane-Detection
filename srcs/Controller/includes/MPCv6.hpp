...
const int N = 14;              // Prediction horizon (increased)
const double V_MAX = 2.5;      // Max velocity (m/s)
const double DELTA_MAX = 0.523; // Max steering angle (rad, 30 deg)
const double DELTA_RATE_MAX = 0.3; // Max steering rate (rad/step, increased)
...
// MPC weights
const double Q_EY = 50.0;      // Weight for cross-track error
const double Q_PSI_ERR = 50.0; // Increased for better curve tracking
...
private:
    ...
    Eigen::Matrix2f Q_; // State cost matrix [ey, yaw]
