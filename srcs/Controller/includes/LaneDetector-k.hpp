#ifndef LANE_DETECTOR_K_HPP
#define LANE_DETECTOR_K_HPP
...
private:
    ...
    bool findLaneEdges(const cv::Mat& lane_mask, const cv::Rect& roi, std::vector<cv::Point>& left_edges, std::vector<cv::Point>& right_edges) const;
    void weightedLinearRegression(const std::vector<cv::Point>& points, double& slope, double& intercept) const;
    void calculateOffsetAndAngle(double left_slope, double left_intercept, double right_slope, double right_intercept,
