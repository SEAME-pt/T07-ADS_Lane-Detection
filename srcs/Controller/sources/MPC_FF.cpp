...
float MPCController::estimateCurvature(float yaw) const
{
    ...
    // const float pseudo_lookahead = 2.0f;          // (m)
    return -yaw / lookahead_;                // (1/m)
}
...
void MPCController::update(float ey, float yaw, float v) {
    ...
    float delta_ff  = k_ff_ * (v_ * v_ / L_) * curvature; // rad
	if (std::abs(curvature) > 1.0 / 0.8) {  // Se raio <0.6m
		delta_ff *= 1.15;  // Boost de 20% para curvas apertadas
	}
    delta_ff = std::clamp(delta_ff, -delta_max, delta_max);
