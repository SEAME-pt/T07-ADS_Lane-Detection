...
MPCController::MPCController(float wheelbase, float dt, int horizon)
    ...
    Q_ << Q_EY, 0.0f, 0.0f, Q_PSI_ERR; // ey: 50, yaw: 40
    R_ = R_DELTA; // 1.0
    Qf_ << 2.0f * Q_EY, 0.0f, 0.0f, 2.0f * Q_PSI_ERR; // ey: 100, yaw: 80
    max_delta_ = DELTA_MAX; // 0.523 rad
    k_delta_ = 0.05f; // Reduced for gentler steering
...
void MPCController::update(float ey, float yaw, float v) {
    ...
    float delta_max = std::min({max_delta_, k_delta_ / v_, 0.08f}); // Cap at 0.08 rad
    ...
    // Constant delta rate penalty for stability
    float R_delta_rate = 50.0f; // Increased for straight-line stability
    for (int i = 0; i < N_ - 1; ++i) {
    ...
    for (int iter = 0; iter < 200; ++iter) {
        ...
        if (cost_new > cost_old && alpha > 0.001f) {
            ...
            u_prev = u;
            alpha = std::min(alpha * 1.2f, 0.02f);
        }
    }
