...
float MPCController::estimateCurvature(float yaw) const
{
    ...
    const float pseudo_lookahead = 5.0f;          // (m)
    return yaw / pseudo_lookahead;                // (1/m)
}
...
void MPCController::update(float ey, float yaw, float v) {
    ...
    for (int iter = 0; iter < MPC_ITER; ++iter) {
        ...
        u = u.cwiseMax(-max_delta_).cwiseMin(max_delta_);
        for (int i = 1; i < N_; ++i) {
    ...
    delta_prev_ = delta_;
    delta_      = std::clamp(delta_ff + u(0), -max_delta_, max_delta_);
